...
class AdaFace(nn.Module):
    def __init__(self, args):
        ...
        checkpoint = torch.load(args.adaface_ckpt, map_location='cpu')
        state_dict = {key[6:]:val for key, val in checkpoint['state_dict'].items() if key.startswith('model.')}
        self.adaface.load_state_dict(state_dict)

    def feature(self, x):
        x = self.adaface.input_layer(x)

        for idx, module in enumerate(self.adaface.body):
            x = module(x)
